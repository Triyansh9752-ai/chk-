...
async def check_card(session, cc, cancel_event=None):
...
async def single_check(cc):
    async with aiohttp.ClientSession() as session:
        result = await check_card(session, format_card(cc))
        return result
...
async def mass_check_with_progress(cards, user_id, progress_callback=None):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        def spawn_task(cc):
            task = asyncio.create_task(check_card(session, cc, cancel_event))
            task._cc = cc
            pending.add(task)
